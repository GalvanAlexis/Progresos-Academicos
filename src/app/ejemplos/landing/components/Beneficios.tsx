    ...
    <section id="beneficios" className="lum-section lum-beneficios">
      <div className="lum-container">
        ...
        <div className="lum-card-grid">
          {BENEFITS.map((b, i) => (
            <div key={b.title} className="lum-card lum-card-reveal" style={{ ['--card-delay' as string]: `${i * 0.15}s` }} >
              <button onClick={() => setSelected(b.title)}
        ...
        {selected && (
          <>
            <div key="ben-backdrop" onClick={() => setSelected(null)}
      ...
      `}</style>
      <script dangerouslySetInnerHTML={{ __html: `{ var cards = document.querySelectorAll('.lum-card'); cards.forEach(function(c) { if (!matchMedia('(hover: hover) and (pointer: fine)').matches) return; c.addEventListener('mousemove', function(e) { var r = c.getBoundingClientRect(); var x = (e.clientX - r.left) / r.width - 0.5; var y = (e.clientY - r.top) / r.height - 0.5; c.style.setProperty('--tilt-x', (-y * 8) + 'deg'); c.style.setProperty('--tilt-y', (x * 8) + 'deg'); }); c.addEventListener('mouseleave', function() { c.style.setProperty('--tilt-x', '0deg'); c.style.setProperty('--tilt-y', '0deg'); }); }); }`, }} />
    </section>
  );
}
